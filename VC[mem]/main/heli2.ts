...
async function body() {
  ...
  await get_next_heli2_checkpoint();  // SCM GOSUB get_next_heli2_checkpoint
  ...
  if ($.player1.isInAnyCar()) {
    ...
    // SCM GOTO → mission_heli2_failed (not lowered; manual jump required)
    throw new Error("unresolved GOTO mission_heli2_failed"); // fallback: would break linear control flow
  }
  ...
  // SCM GOTO → mission_heli2_passed (not lowered; manual jump required)
  throw new Error("unresolved GOTO mission_heli2_passed"); // fallback: would break linear control flow
}
...
async function mission_heli2_passed() {
  ...
  Stat.RegisterFastestTime(13, $.heli_timer);
...
async function get_next_heli2_checkpoint() {
  ///////////////////////////////////////////////
...
export async function heli2() {
  // Mission start stuff


  await body();  // SCM GOSUB mission_start_heli2


  if (HAS_DEATHARREST_BEEN_EXECUTED()) {
    await onFailed();  // SCM GOSUB mission_heli2_failed
  }


  await cleanup();  // SCM GOSUB mission_cleanup_heli2
