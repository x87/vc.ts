...
async function haitian_factory_gates() {
...
async function haitian_factory_gate_inner() {
  // SCM GOTO → haitian_factory_gate_inner lowered to endless loop
  while (true) {
    await asyncWait(1000);
    if (andy_has_control == 0) {
      if ($.player1.isPlaying()) {
        if ($.player1.isInZone("HAITI")) {
          if ($.player1.isInArea3D(-1107.0, 60.0, 5.0, -1121.0, 80.0, 15.0, false /* FALSE */)) {
            ...
            $.gate_slide_loop.remove();
            [objectx, objecty, objectz] = $.haitian_gate.getCoordinates();
            if (gate_sfx2_hait == 1) {
              not_clunked_yet = 1;
