// Generated from main/cuban1.sc

import { $ } from "../utils/vars.mts";

async function mission_start_cuban1() {
  Text.ClearThisPrint("M_FAIL");
  ONMISSION = 1;
  Stat.RegisterMissionGiven();
  await asyncWait(0);
  Text.LoadMissionText("CUBAN1");
  Shortcut.SetDropoffPointForMission(-1173.5, -599.6, 10.2, 277.1);
  // *****************************************Set Flags/variables************************************


  $.boatcheck_blipx = 0.0;
  $.boatcheck_blipy = 0.0;
  $.boatcheck_blipz = 0.0;


  $.next_boatcheck_blipx = 0.0;
  $.next_boatcheck_blipy = 0.0;
  $.next_boatcheck_blipz = 0.0;


  $.boatrace_end = 181000;


  $.boat_check_counter = 0;


  $.del_ramps = 0;
  $.jumpy_flag = 0;
  $.boat_col_check = 0;
  $.boat_speed = 0.0;
  $.time_check = 0;
  $.starting_timer = 0;
  $.cub1_speech_played = 0;
  $.checks_left = 27;
  $.golfer_hit1 = 0;
  $.golfer_hit2 = 0;
  $.golfer_hit3 = 0;
  $.golfer_hit4 = 0;

  //CHECKPOINTS


  $.boat_check1x = -102.9375;
  $.boat_check1y = 171.7835;
  $.boat_check1z = 5.6727;


  $.boat_check2x = -64.7472;
  $.boat_check2y = 368.7051;
  $.boat_check2z = 5.5050;


  $.boat_check3x = -27.916;
  $.boat_check3y = 482.466;
  $.boat_check3z = 16.0159;


  $.boat_check4x = 183.4692;
  $.boat_check4y = 614.9838;
  $.boat_check4z = 5.7676;


  $.boat_check5x = 239.9839;
  $.boat_check5y = 422.9632;
  $.boat_check5z = 5.9437;


  $.boat_check6x = 196.5818;
  $.boat_check6y = 227.5221;
  $.boat_check6z = 5.9738;


  $.boat_check7x = 336.4845;
  $.boat_check7y = 83.6320;
  $.boat_check7z = 16.0888;


  $.boat_check8x = 417.2411;
  $.boat_check8y = -100.5402;
  $.boat_check8z = 15.9255;


  $.boat_check9x = 402.8727;
  $.boat_check9y = -300.6653;
  $.boat_check9z = 5.7619;


  $.boat_check10x = 290.0335;
  $.boat_check10y = -460.3196;
  $.boat_check10z = 5.9769;


  $.boat_check11x = 181.6709;
  $.boat_check11y = -617.7477;
  $.boat_check11z = 5.9921;


  $.boat_check12x = -4.6467;
  $.boat_check12y = -694.4938;
  $.boat_check12z = 5.8744;


  $.boat_check13x = -191.9626;
  $.boat_check13y = -772.1127;
  $.boat_check13z = 5.8744;


  $.boat_check14x = -335.464;
  $.boat_check14y = -910.510;
  $.boat_check14z = 5.4899;


  $.boat_check15x = -304.464;
  $.boat_check15y = -988.510;
  $.boat_check15z = 5.4899;


  $.boat_check16x = -244.464;
  $.boat_check16y = -922.510;
  $.boat_check16z = 5.4899;


  $.boat_check17x = -144.779;
  $.boat_check17y = -845.288;
  $.boat_check17z = 6.0213;


  $.boat_check18x = 26.0794;
  $.boat_check18y = -739.893;
  $.boat_check18z = 6.0163;


  $.boat_check19x = 154.4145;
  $.boat_check19y = -640.624;
  $.boat_check19z = 5.3606;


  $.boat_check20x = 304.531;
  $.boat_check20y = -431.591;
  $.boat_check20z = 16.0599;


  $.boat_check21x = 413.1744;
  $.boat_check21y = -280.507;
  $.boat_check21z = 5.9702;


  $.boat_check22x = 423.1779;
  $.boat_check22y = -89.6841;
  $.boat_check22z = 15.8592;


  $.boat_check23x = 339.5622;
  $.boat_check23y = 73.0977;
  $.boat_check23z = 6.0374;


  $.boat_check24x = 198.6796;
  $.boat_check24y = 243.2925;
  $.boat_check24z = 5.5461;


  $.boat_check25x = 75.6552;
  $.boat_check25y = 401.3978;
  $.boat_check25z = 5.6628;


  $.boat_check26x = -95.1011;
  $.boat_check26y = 295.7863;
  $.boat_check26z = 5.8772;


  $.boat_check27x = -86.3112;
  $.boat_check27y = 95.7909;
  $.boat_check27z = 5.6436;


  $.camerax = -4.56;
  $.cameray = 456.21;
  $.cameraz = 11.51;
  $.locate1x = -42.74;
  $.locate1y = 460.99;
  $.locate2x = -20.19;
  $.locate2y = 464.61;

  // ****************************************START OF CUTSCENE********************************

  Streaming.SetAreaVisible(7 /* VIS_COFFEE_SHOP */);
  $.player1.setMood(0 /* PLAYER_MOOD_CALM */, 60000);


  World.ClearAreaOfChars(-1174.8, -609.0, 10.4, -116.3, -632.5, 15.3);
  World.SetCarDensityMultiplier(0.0);
  World.SetPedDensityMultiplier(0.0);


  Streaming.LoadSpecialCharacter(1, "CSplay");
  Streaming.LoadSpecialCharacter(2, "CSpablo");
  Streaming.LoadSpecialCharacter(3, "CSpapa");
  Streaming.LoadSpecialCharacter(4, "CSpepe");
  Streaming.LoadSpecialCharacter(5, "CSumbto");


  Streaming.LoadSpecialModel(CUTOBJ01, "ccfan");
  Streaming.LoadSpecialModel(CUTOBJ02, "lobtray");
  Streaming.LoadSpecialModel(CUTOBJ03, "espreso");


  Streaming.LoadScene(-1169.0, -620.0, 10.791);


  Streaming.LoadAllModelsNow();


  while (!(Streaming.HasSpecialCharacterLoaded(1)) || !(Streaming.HasSpecialCharacterLoaded(2)) || !(Streaming.HasSpecialCharacterLoaded(3)) || !(Streaming.HasSpecialCharacterLoaded(4)) || !(Streaming.HasSpecialCharacterLoaded(5))) {
    await asyncWait(0);
  }


  while (!(Streaming.HasModelLoaded(CUTOBJ01)) || !(Streaming.HasModelLoaded(CUTOBJ02))) {
    await asyncWait(0);
  }


  Cutscene.Load("cub_1");
  Cutscene.SetOffset(-1170.708, -608.327, 10.791);
  Camera.SetNearClip(0.1);


  $.cs_player = CutsceneObject.Create(SPECIAL01);
  $.cs_player.setAnim("CSplay");


  $.cs_pablo = CutsceneObject.Create(SPECIAL02);
  $.cs_pablo.setAnim("CSpablo");


  $.cs_papa = CutsceneObject.Create(SPECIAL03);
  $.cs_papa.setAnim("CSpapa");


  $.cs_pepe = CutsceneObject.Create(SPECIAL04);
  $.cs_pepe.setAnim("CSpepe");


  $.cs_umbto = CutsceneObject.Create(SPECIAL05);
  $.cs_umbto.setAnim("CSumbto");


  $.cc_fan1 = CutsceneObject.Create(CUTOBJ01);
  Cutscene.SetAnimToLoop("ccfan");
  $.cc_fan1.setAnim("ccfan");


  $.cs_lobtray = CutsceneObject.Create(CUTOBJ02);
  $.cs_lobtray.setAnim("lobtray");


  $.cs_espreso = CutsceneObject.Create(CUTOBJ03);
  $.cs_espreso.setAnim("espreso");


  World.ClearArea(-1170.9, -603.7, 10.6, 1.0, true /* TRUE */);
  $.player1.setCoordinates(-1170.9, -603.7, 10.6);


  Camera.DoFade(1500, 1 /* FADE_IN */);


  World.SwitchRubbish(false /* OFF */);
  Streaming.Switch(true /* ON */);
  Cutscene.Start();

  // Displays cutscene text


  $.cs_time = Cutscene.GetTime();


  while ($.cs_time < 2145) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_A", 10000, 1); // Si, men?


  while ($.cs_time < 3036) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_B", 10000, 1); // Hey, easy poppa, this man for me. You, you the boy?


  while ($.cs_time < 10501) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_C", 10000, 1); // Oh yeh. You the boy. I think so, you know?


  while ($.cs_time < 14480) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_D", 10000, 1); // No. I don't think I do.


  while ($.cs_time < 16593) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_E", 10000, 1); // Oh yeah? You come here, tough guy.


  while ($.cs_time < 19202) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_F", 10000, 1); // You think you can a take me on?


  while ($.cs_time < 20637) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_G", 10000, 1); // You think you can play stupid with me?


  while ($.cs_time < 22441) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_H", 10000, 1); // No, I think you're playing plenty stupid enough for the both of us.


  while ($.cs_time < 24775) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_I", 10000, 1); // Hey, he call you dumb, son.


  while ($.cs_time < 27472) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_J", 10000, 1); // And I call him a little girl, poppa.


  while ($.cs_time < 30348) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_K", 10000, 1); // Look at him, all dressed up like that.


  while ($.cs_time < 33133) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_L", 10000, 1); // What is this, ladies night?


  while ($.cs_time < 35329) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_M", 10000, 1); // You some big tough guy, you dress like a woman?


  while ($.cs_time < 38022) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_N", 10000, 1); // You got panties like a woman too, huh?


  while ($.cs_time < 40251) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_O", 10000, 1); // What you got against women? You prefer men, big boy?


  while ($.cs_time < 42742) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_P", 10000, 1); // I like women! I like all women! I love my mother, Chico.


  while ($.cs_time < 46831) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_Q", 10000, 1); // Alright, I'll take your word for it.


  while ($.cs_time < 51281) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_R", 10000, 1); // Can you drive, amigo?


  while ($.cs_time < 53403) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_S", 10000, 1); // Yeah... like a woman.


  while ($.cs_time < 57539) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_T", 10000, 1); // Very funny. I like you, big boy. Maybe you can help.


  while ($.cs_time < 62464) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_U", 10000, 1); // Maybe you can prove you a man.


  while ($.cs_time < 65377) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_V", 10000, 1); // Take out the boat.


  while ($.cs_time < 68974) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_W", 10000, 1); // Show me you got some big cajones,


  while ($.cs_time < 71656) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Text.PrintNow("CUB1_X", 10000, 1); // and not some little Chiquita ones.
  /*
  WHILE cs_time < 72656
  WAIT 0
  GET_CUTSCENE_TIME cs_time
  ENDWHILE

  CLEAR_PRINTS
  */

  while ($.cs_time < 75392) {
    await asyncWait(0);
    $.cs_time = Cutscene.GetTime();
  }


  Camera.DoFade(1500, 0 /* FADE_OUT */);


  Text.ClearPrints();


  while (Camera.GetFadingStatus()) {
    await asyncWait(0);
  }


  World.SwitchRubbish(true /* ON */);
  Cutscene.Clear();


  Streaming.UnloadSpecialCharacter(1);
  Streaming.UnloadSpecialCharacter(2);
  Streaming.UnloadSpecialCharacter(3);
  Streaming.UnloadSpecialCharacter(4);
  Streaming.UnloadSpecialCharacter(5);


  Streaming.MarkModelAsNoLongerNeeded(CUTOBJ01);
  Streaming.MarkModelAsNoLongerNeeded(CUTOBJ02);
  Streaming.MarkModelAsNoLongerNeeded(CUTOBJ03);
  Streaming.SetAreaVisible(0 /* VIS_MAIN_MAP */);
  $.flag_player_in_cafe = 0;
  World.SetCarDensityMultiplier(1.0);
  World.SetPedDensityMultiplier(1.0);
  // ******************************************END OF CUTSCENE********************************

  Camera.SetFadingColor(0, 0, 0);


  await asyncWait(500);


  Clock.SetTimeOfDay(10, 0);
  //------------------REQUEST_MODELS ------------------------------

  Streaming.RequestModel(speeder);
  Streaming.RequestModel(CBb);
  Streaming.RequestModel(caddy);
  Streaming.RequestModel(WMYGO);
  Streaming.RequestModel(WFOGO);
  Streaming.RequestModel(reefer);


  while (!(Streaming.HasModelLoaded(speeder)) || !(Streaming.HasModelLoaded(CBb)) || !(Streaming.HasModelLoaded(caddy)) || !(Streaming.HasModelLoaded(WMYGO)) || !(Streaming.HasModelLoaded(WFOGO)) || !(Streaming.HasModelLoaded(reefer))) {
    await asyncWait(0);
  }


  Audio.LoadMissionAudio(1, "CUB1_9" as any);
  Audio.LoadMissionAudio(2, "COL3_2b" as any);


  while (!(Audio.HasMissionAudioLoaded(1)) || !(Audio.HasMissionAudioLoaded(2))) {
    await asyncWait(0);
  }

  //forcing weather to be nice

  Weather.ForceNow(4 /* WEATHER_EXTRA_SUNNY */);

  //switching off ped nodes

  Path.SwitchRoadsOff(130.4, 368.4, 50.7, -228.4, 724.5, 0.0);


  /////////////test/////////////////////////////

  Streaming.LoadScene(-74.17, 90.20, 9.67);
  Streaming.RequestCollision(-67.7, 91.2);
  $.player1.setCoordinates(-67.7, 91.2, 9.5);
  //creating boat and waiting for player to reach it

  $.rico_passenger = Char.Create(7 /* PEDTYPE_GANG_CUBAN */, CBb, -74.17, 90.20, 8.6);
  $.rico_passenger.setAsPlayerFriend($.player1, true /* TRUE */);
  $.rico_passenger.setHeading(40.2);

  //creating golf fluff

  $.fake_caddy = Car.Create(caddy, -31.7, 484.9, 7.3);
  $.fake_caddy.setHeading(205.8);
  $.fake_golfer1 = Char.Create(4 /* PEDTYPE_CIVMALE */, WMYGO, -29.5, 483.6, 7.7);
  $.fake_golfer1.setHeading(11.0);
  $.fake_golfer1.setHealth(10);
  $.fake_golfer2 = Char.Create(4 /* PEDTYPE_CIVMALE */, WMYGO, -27.2, 484.1, 7.7);
  $.fake_golfer2.setHeading(77.0);
  $.fake_golfer2.setHealth(10);
  $.fake_golfer3 = Char.Create(5 /* PEDTYPE_CIVFEMALE */, WFOGO, -29.9, 484.4, 7.7);
  $.fake_golfer3.setHeading(282.0);
  $.fake_golfer3.setHealth(10);
  $.fake_golfer4 = Char.Create(5 /* PEDTYPE_CIVFEMALE */, WFOGO, -28.4, 486.3, 7.7);
  $.fake_golfer4.setHeading(176.0);
  $.fake_golfer4.setHealth(10);

  /*
  CREATE_CAR reefer -192.0 -771.1 5.4 fake_reefer
  SET_CAR_HEADING fake_reefer 181.0
  ANCHOR_BOAT fake_reefer TRUE
  */

  Game.SetCharsChatting($.fake_golfer1, $.fake_golfer2, 86400000);
  Game.SetCharsChatting($.fake_golfer3, $.fake_golfer4, 86400000);


  Camera.DoFade(1500, 1 /* FADE_IN */);
  // ******************************************START OF PLAYER GETTING INTO BOAT*******************************************

  $.player1.setControl(false /* OFF */);


  Hud.SwitchWidescreen(true /* ON */);


  Camera.SetFixedPosition(-67.34, 85.31, 10.02, 0.0, 0.0, 0.0);
  Camera.PointAtPoint(-86.40, 117.01, 13.04, 2 /* JUMP_CUT */);


  await asyncWait(1000);


  if (!(Char.IsDead($.rico_passenger))) {
    $.scplayer.setObjGotoCharOnFoot($.rico_passenger);
  }


  await asyncWait(2000);


  Text.PrintNow("CUB1_01", 4000, 1); //Hey, I'm Rico. You the man with the cajones?
  Audio.PlayMissionAudio(1);
  while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
    await asyncWait(0);
    if (Pad.IsButtonPressed(0 /* PAD1 */, 16 /* CROSS */)) {
      if ($.button_pressed == 1) {
        $.button_pressed = 0;
        // SCM GOTO → end_cut_cub1_1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO end_cut_cub1_1"); // fallback: would break linear control flow
      }
    }
    else {
      if ($.button_pressed == 0) {
        $.button_pressed = 1;
      }
    }
  }
  Audio.ClearMissionAudio(1);
  Text.ClearThisPrint("CUB1_01");

  //SET_CHAR_WAIT_STATE scplayer WAITSTATE_PLAYANIM_CHAT 10000


  Text.PrintNow("GEN3_4", 4000, 1); //Tommy Vercetti. Let's go...
  Audio.PlayMissionAudio(2);
  while (!(Audio.HasMissionAudioFinished(2))) {
    await asyncWait(0);
    if (Pad.IsButtonPressed(0 /* PAD1 */, 16 /* CROSS */)) {
      if ($.button_pressed == 1) {
        $.button_pressed = 0;
        // SCM GOTO → end_cut_cub1_1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO end_cut_cub1_1"); // fallback: would break linear control flow
      }
    }
    else {
      if ($.button_pressed == 0) {
        $.button_pressed = 1;
      }
    }
  }
  Audio.ClearMissionAudio(2);
  Text.ClearThisPrint("GEN3_4");

  //SET_CHAR_WAIT_STATE scplayer WAITSTATE_FALSE 100


  Audio.LoadMissionAudio(1, "CUB1_10" as any);


  while (!(Audio.HasMissionAudioLoaded(1))) {
    await asyncWait(0);
  }


  Text.PrintNow("CUB1_02", 4000, 1); //Ok man, treat her like a woman.
  Audio.PlayMissionAudio(1);
  while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
    await asyncWait(0);
    if (Pad.IsButtonPressed(0 /* PAD1 */, 16 /* CROSS */)) {
      if ($.button_pressed == 1) {
        $.button_pressed = 0;
        // SCM GOTO → end_cut_cub1_1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO end_cut_cub1_1"); // fallback: would break linear control flow
      }
    }
    else {
      if ($.button_pressed == 0) {
        $.button_pressed = 1;
      }
    }
  }
  Audio.ClearMissionAudio(1);
  Text.ClearThisPrint("CUB1_02");
}



async function end_cut_cub1_1() {
  Audio.ClearMissionAudio(1);
  Audio.ClearMissionAudio(2);
  Text.ClearSmallPrints();
  //SET_CHAR_WAIT_STATE scplayer WAITSTATE_FALSE 100
  //IF NOT IS_CHAR_DEAD rico_passenger
  //SET_CHAR_WAIT_STATE rico_passenger WAITSTATE_FALSE 100
  //ENDIF

  Camera.RestoreJumpcut();
  Hud.SwitchWidescreen(false /* OFF */);
  // ******************************************END OF PLAYER GETTING INTO BOAT*********************************************

  //FADE OUT

  Camera.SetFadingColor(1, 1, 1);
  Camera.DoFade(1500, 0 /* FADE_OUT */);


  while (Camera.GetFadingStatus()) {
    await asyncWait(0);
  }


  $.pred_boat = Car.Create(speeder, -90.9, 105.1, 5.0);
  $.pred_boat.setHeading(9.2);
  $.pred_boat.anchor(true /* TRUE */);


  if (!(Car.IsDead($.pred_boat))) {
    $.player1.warpIntoCar($.pred_boat);
    if (!(Char.IsDead($.rico_passenger))) {
      $.rico_passenger.attachToCar($.pred_boat, 0.0, 1.8, 1.2, 0 /* FACING_FORWARD */, 360.0, 0 /* WEAPONTYPE_UNARMED */); ////speeder
      $.rico_passenger.turnToFaceChar($.scplayer);
    }
    else {
      Text.PrintNow("CUB1_11", 5000, 1); //You Killed Rico!
      Camera.DoFade(1500, 1 /* FADE_IN */);
      // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
      throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
    }
  }
  else {
    Text.PrintNow("CUB1_11", 5000, 1); //You Killed Rico!
    Camera.DoFade(1500, 1 /* FADE_IN */);
    // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
    throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
  }


  Camera.SetBehindPlayer();
  $.player1.setControl(true /* ON */);


  //setting up checkpoints

  $.boatcheck_blipx = $.boat_check1x;
  $.boatcheck_blipy = $.boat_check1y;
  $.boatcheck_blipz = $.boat_check1z;
  $.next_boatcheck_blipx = $.boat_check2x;
  $.next_boatcheck_blipy = $.boat_check2y;
  $.next_boatcheck_blipz = $.boat_check2z;


  $.boatcheck_blip = Blip.AddForCoord($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz);
  $.boatcheck_blip.changeColor(4 /* YELLOW */);
  $.next_boatcheck_blip = Blip.AddForCoord($.next_boatcheck_blipx, $.next_boatcheck_blipy, $.next_boatcheck_blipz);
  $.next_boatcheck_blip.changeColor(4 /* YELLOW */);
  $.next_boatcheck_blip.changeScale(2);
  $.next_boatcheck_blip.dim(true /* TRUE */);

  //creating fluff

  $.ramp1 = ScriptObject.Create(waterjumpx2, -34.26, 455.90, 5.5);
  $.ramp1.setHeading(343.60);
  $.ramp1.setCollision(true /* TRUE */);


  $.ramp2 = ScriptObject.Create(waterjumpx2, 312.200, 107.669, 5.5);
  $.ramp2.setHeading(230.88);
  $.ramp2.setCollision(true /* TRUE */);


  $.ramp3 = ScriptObject.Create(waterjumpx2, 414.5, -79.8, 5.5);
  $.ramp3.setHeading(184.9);
  $.ramp3.setCollision(true /* TRUE */);

  /*
  CREATE_OBJECT waterjump1 -176.0 -763.5 5.5 ramp4
  SET_OBJECT_HEADING ramp4 121.4
  SET_OBJECT_COLLISION ramp4 TRUE
  */


  Camera.DoFade(1500, 1 /* FADE_IN */);



  Text.PrintNow("CUB1_12", 5000, 1); //Go through the first checkpoint to begin the test.
  Text.Print("CUB1_13", 5000, 1); //You have three minutes to get round the course.
}




async function boat_loop() {
  // SCM GOTO → boat_loop lowered to endless loop
  while (true) {


    await asyncWait(0);


    World.ClearAreaOfCars(-185.35, -738.31, 0.0, -150.63, -785.11, 15.0);


    if ($.boat_check_counter < 4) {
      if ($.golfer_hit1 == 0) {
        if (Char.IsDead($.fake_golfer1)) {
          Text.PrintWithNumberBig("BONUS", 100, 1000, 1);
          Sound.AddOneOffSound(0.0, 0.0, 0.0, 1 /* SOUND_PART_MISSION_COMPLETE */);
          $.player1.addScore(100);
          $.golfer_hit1 = 1;
        }
      }
      /*
      IF golfer_hit2 = 0
      IF IS_CHAR_DEAD fake_golfer2
      PRINT_WITH_NUMBER_BIG BONUS 100 1000 1
      ADD_ONE_OFF_SOUND 0.0 0.0 0.0 SOUND_PART_MISSION_COMPLETE
      ADD_SCORE player1 100
      golfer_hit2 = 1
      ENDIF
      ENDIF
      IF golfer_hit3 = 0
      IF IS_CHAR_DEAD fake_golfer3
      PRINT_WITH_NUMBER_BIG BONUS 100 1000 1
      ADD_ONE_OFF_SOUND 0.0 0.0 0.0 SOUND_PART_MISSION_COMPLETE
      ADD_SCORE player1 100
      golfer_hit3 = 1
      ENDIF
      ENDIF
      IF golfer_hit4 = 0
      IF IS_CHAR_DEAD fake_golfer4
      PRINT_WITH_NUMBER_BIG BONUS 100 1000 1
      ADD_ONE_OFF_SOUND 0.0 0.0 0.0 SOUND_PART_MISSION_COMPLETE
      ADD_SCORE player1 100
      golfer_hit4 = 1
      ENDIF
      ENDIF
      */

    }



    if ($.boat_check_counter == 4) {
      if (!(Char.IsDead($.fake_golfer1))) {
        $.fake_golfer1.setStayInSamePlace(true /* TRUE */);
      }
      if (!(Char.IsDead($.fake_golfer2))) {
        $.fake_golfer2.setStayInSamePlace(true /* TRUE */);
      }
      if (!(Char.IsDead($.fake_golfer3))) {
        $.fake_golfer3.setStayInSamePlace(true /* TRUE */);
      }
      if (!(Char.IsDead($.fake_golfer4))) {
        $.fake_golfer4.setStayInSamePlace(true /* TRUE */);
      }
    }


    if (!(Car.IsDead($.pred_boat))) {
      if ($.player1.isInArea2D($.locate1x, $.locate1y, $.locate2x, $.locate2y, false /* FALSE */)) {
        if ($.boat_check_counter == 2) {
          if (!(Char.IsDead($.fake_golfer1))) {
            $.fake_golfer1.setObjFleeOnFootTillSafe();
          }
          if (!(Char.IsDead($.fake_golfer2))) {
            $.fake_golfer2.setObjFleeOnFootTillSafe();
          }
          if (!(Char.IsDead($.fake_golfer3))) {
            $.fake_golfer3.setObjFleeOnFootTillSafe();
          }
          if (!(Char.IsDead($.fake_golfer4))) {
            $.fake_golfer4.setObjFleeOnFootTillSafe();
          }
        }
        if ($.player1.isInAnyCar()) {
          $.preddy_boat = $.player1.storeCarIsInNoSave();
          $.boat_speed = $.preddy_boat.getSpeed();
          if (!($.preddy_boat.isInWater())) {
            if ($.boat_speed > 15.0) {
              $.jumpy_flag = 1;
            }
          }
          if ($.jumpy_flag == 1) {
            Clock.SetTimeScale(0.3);
            Camera.SetFixedPosition($.camerax, $.cameray, $.cameraz, 0.0, 0.0, 0.0);
            Camera.PointAtCar($.preddy_boat, 15 /* FIXED */, 2 /* JUMP_CUT */);
            while (!($.preddy_boat.isInWater())) {
              await asyncWait(0);
              $.time_check++;
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
              if ($.boat_col_check == 0) {
                if ($.player1.locateAnyMeans3D($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 7.0, 7.0, 7.0, false /* FALSE */)) {
                  $.boatcheck_blip.remove();
                  $.next_boatcheck_blip.remove();
                  ++$.boat_check_counter;
                  --$.checks_left;
                  Sound.AddOneOffSound(0.0, 0.0, 0.0, 1 /* SOUND_PART_MISSION_COMPLETE */);
                  if ($.boat_check_counter == 3) {
                    $.camerax = 369.99;
                    $.cameray = 74.30;
                    $.cameraz = 8.66;
                    $.locate1x = 308.57;
                    $.locate1y = 99.09;
                    $.locate2x = 340.835;
                    $.locate2y = 107.66;
                    $.boatcheck_blipx = $.boat_check4x;
                    $.boatcheck_blipy = $.boat_check4y;
                    $.boatcheck_blipz = $.boat_check4z;
                    $.next_boatcheck_blipx = $.boat_check5x;
                    $.next_boatcheck_blipy = $.boat_check5y;
                    $.next_boatcheck_blipz = $.boat_check5z;
                  }
                  if ($.boat_check_counter == 7) {
                    $.camerax = 396.14;
                    $.cameray = -90.01;
                    $.cameraz = 11.48;
                    $.locate1x = 421.73;
                    $.locate1y = -83.21;
                    $.locate2x = 403.83;
                    $.locate2y = -91.97;
                    $.boatcheck_blipx = $.boat_check8x;
                    $.boatcheck_blipy = $.boat_check8y;
                    $.boatcheck_blipz = $.boat_check8z;
                    $.next_boatcheck_blipx = $.boat_check9x;
                    $.next_boatcheck_blipy = $.boat_check9y;
                    $.next_boatcheck_blipz = $.boat_check9z;
                  }
                  if ($.boat_check_counter == 8) {
                    $.camerax = -194.5;
                    $.cameray = -783.5;
                    $.cameraz = 9.2;
                    $.locate1x = -183.166;
                    $.locate1y = -756.844;
                    $.locate2x = -181.80;
                    $.locate2y = -780.26;
                    $.boatcheck_blipx = $.boat_check9x;
                    $.boatcheck_blipy = $.boat_check9y;
                    $.boatcheck_blipz = $.boat_check9z;
                    $.next_boatcheck_blipx = $.boat_check10x;
                    $.next_boatcheck_blipy = $.boat_check10y;
                    $.next_boatcheck_blipz = $.boat_check10z;
                  }
                  if ($.boat_check_counter == 20) {
                    $.camerax = 435.65;
                    $.cameray = -98.08;
                    $.cameraz = 11.97;
                    $.locate1x = 409.80;
                    $.locate1y = -109.11;
                    $.locate2x = 433.46;
                    $.locate2y = -100.21;
                    $.boatcheck_blipx = $.boat_check21x;
                    $.boatcheck_blipy = $.boat_check21y;
                    $.boatcheck_blipz = $.boat_check21z;
                    $.next_boatcheck_blipx = $.boat_check22x;
                    $.next_boatcheck_blipy = $.boat_check22y;
                    $.next_boatcheck_blipz = $.boat_check22z;
                  }
                  if ($.boat_check_counter == 22) {
                    $.boatcheck_blipx = $.boat_check23x;
                    $.boatcheck_blipy = $.boat_check23y;
                    $.boatcheck_blipz = $.boat_check23z;
                    $.next_boatcheck_blipx = $.boat_check24x;
                    $.next_boatcheck_blipy = $.boat_check24y;
                    $.next_boatcheck_blipz = $.boat_check24z;
                  }
                  $.boatcheck_blip = Blip.AddForCoord($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz);
                  $.boatcheck_blip.changeColor(4 /* YELLOW */);
                  $.next_boatcheck_blip = Blip.AddForCoord($.next_boatcheck_blipx, $.next_boatcheck_blipy, $.next_boatcheck_blipz);
                  $.next_boatcheck_blip.changeColor(4 /* YELLOW */);
                  $.next_boatcheck_blip.changeScale(2);
                  $.next_boatcheck_blip.dim(true /* TRUE */);
                  $.boat_col_check = 1;
                }
              }
              if (Car.IsDead($.preddy_boat)) {
                // SCM GOTO → restore_if_failed (not lowered; manual jump required)
                throw new Error("unresolved GOTO restore_if_failed"); // fallback: would break linear control flow
              }
              if (!($.player1.isPlaying())) {
                // SCM GOTO → restore_if_failed (not lowered; manual jump required)
                throw new Error("unresolved GOTO restore_if_failed"); // fallback: would break linear control flow
              }
              if (!($.player1.isInAnyCar())) {
                // SCM GOTO → restore_if_failed (not lowered; manual jump required)
                throw new Error("unresolved GOTO restore_if_failed"); // fallback: would break linear control flow
              }
              if ($.preddy_boat.isStopped()) {
                // SCM GOTO → restore_if_failed (not lowered; manual jump required)
                throw new Error("unresolved GOTO restore_if_failed"); // fallback: would break linear control flow
              }
              if ($.time_check > 1000) {
                // SCM GOTO → restore_if_failed (not lowered; manual jump required)
                throw new Error("unresolved GOTO restore_if_failed"); // fallback: would break linear control flow
              }
            }
            // SCM label restore_if_failed
            Clock.SetTimeScale(1.0);
            Camera.RestoreJumpcut();
            $.jumpy_flag = 0;
            $.boat_col_check = 0;
            $.time_check = 0;
          }
        }
      }
    }


    if ($.boat_check_counter == 26) {
      Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 255, 0, 0);
    }
    else {
      Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
    }


    if ($.boat_check_counter == 1) {
      if ($.starting_timer == 0) {
        Hud.DisplayTimerWithString($.$id.boatrace_end, 1 /* TIMER_DOWN */, "RACES");
        Hud.DisplayCounterWithString($.$id.checks_left, 0, "KICK1_9");
        $.starting_timer = 1;
      }
    }


    if (!(Car.IsDead($.pred_boat))) {
      if ($.player1.isInCar($.pred_boat)) {
        if ($.cub1_speech_played == 0) {
          if ($.boat_check_counter > 1) {
            Audio.LoadMissionAudio(1, "CUB1_3" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_05", 4000, 1); //Amigo, you a man, man.
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(19 /* WAITSTATE_PLAYANIM_CHAT */, 10000);
            }
            Audio.PlayMissionAudio(1);
            while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Audio.ClearMissionAudio(1);
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(0 /* WAITSTATE_FALSE */, 100);
            }
            Text.ClearThisPrint("CUB1_05");
            $.cub1_speech_played = 1;
          }
        }
        if ($.cub1_speech_played == 1) {
          if ($.boat_check_counter > 4) {
            Audio.LoadMissionAudio(1, "CUB1_4" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_06", 4000, 1); //Call yourself a man, man?
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(19 /* WAITSTATE_PLAYANIM_CHAT */, 10000);
            }
            Audio.PlayMissionAudio(1);
            while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Audio.ClearMissionAudio(1);
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(0 /* WAITSTATE_FALSE */, 100);
            }
            Text.ClearThisPrint("CUB1_06");
            $.cub1_speech_played = 2;
          }
        }
        if ($.cub1_speech_played == 2) {
          if ($.boat_check_counter > 9) {
            Audio.LoadMissionAudio(1, "CUB1_5" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_07", 4000, 1); //You a little scaredy kitten, baby boy, go cry to your mommy?
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(19 /* WAITSTATE_PLAYANIM_CHAT */, 10000);
            }
            Audio.PlayMissionAudio(1);
            while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Audio.ClearMissionAudio(1);
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(0 /* WAITSTATE_FALSE */, 100);
            }
            Text.ClearThisPrint("CUB1_07");
            $.cub1_speech_played = 3;
          }
        }
        if ($.cub1_speech_played == 3) {
          if ($.boat_check_counter > 15) {
            Audio.LoadMissionAudio(1, "CUB1_1" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_03", 4000, 1); //Not bad, you a real man.
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(19 /* WAITSTATE_PLAYANIM_CHAT */, 10000);
            }
            Audio.PlayMissionAudio(1);
            while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Audio.ClearMissionAudio(1);
            if (!(Char.IsDead($.rico_passenger))) {
              $.rico_passenger.setWaitState(0 /* WAITSTATE_FALSE */, 100);
            }
            Text.ClearThisPrint("CUB1_03");
            $.cub1_speech_played = 4;
          }
        }
        if ($.cub1_speech_played == 4) {
          if ($.boat_check_counter > 22) {
            Audio.LoadMissionAudio(1, "CUB1_7" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_09", 4000, 1); //Man, you the man, man. I like you, man. I like you a lot.


            /*
            IF NOT IS_CHAR_DEAD rico_passenger
            SET_CHAR_WAIT_STATE rico_passenger WAITSTATE_PLAYANIM_CHAT 10000
            ENDIF
            */
            Audio.PlayMissionAudio(1);
            /*
            WHILE NOT HAS_MISSION_AUDIO_FINISHED 1
            AND NOT IS_CHAR_DEAD rico_passenger
            WAIT 0
            DRAW_CORONA boatcheck_blipx boatcheck_blipy boatcheck_blipz 5.0 CORONATYPE_CIRCLE FLARETYPE_NONE 150 150 0
            ENDWHILE
            CLEAR_MISSION_AUDIO 1

            IF NOT IS_CHAR_DEAD rico_passenger
            SET_CHAR_WAIT_STATE rico_passenger WAITSTATE_FALSE 100
            ENDIF

            CLEAR_THIS_PRINT CUB1_09
            */
            $.cub1_speech_played = 5;
          }
        }
        if ($.cub1_speech_played == 5) {
          if ($.boat_check_counter > 23) {
            Audio.LoadMissionAudio(1, "CUB1_8" as any);
            while (!(Audio.HasMissionAudioLoaded(1))) {
              await asyncWait(0);
              Fx.DrawCorona($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 5.0, 6 /* CORONATYPE_CIRCLE */, 0 /* FLARETYPE_NONE */, 150, 150, 0);
            }
            Text.PrintNow("CUB1_10", 4000, 1); //Any time, man. 'cause you got cajones. And all my friends have big cajones.

            /*
            IF NOT IS_CHAR_DEAD rico_passenger
            SET_CHAR_WAIT_STATE rico_passenger WAITSTATE_PLAYANIM_CHAT 10000
            ENDIF
            */
            Audio.PlayMissionAudio(1);
            /*
            WHILE NOT HAS_MISSION_AUDIO_FINISHED 1
            AND NOT IS_CHAR_DEAD rico_passenger
            WAIT 0
            DRAW_CORONA boatcheck_blipx boatcheck_blipy boatcheck_blipz 5.0 CORONATYPE_CIRCLE FLARETYPE_NONE 150 150 0
            ENDWHILE
            CLEAR_MISSION_AUDIO 1

            IF NOT IS_CHAR_DEAD rico_passenger
            SET_CHAR_WAIT_STATE rico_passenger WAITSTATE_FALSE 100
            ENDIF

            CLEAR_THIS_PRINT CUB1_10
            */
            $.cub1_speech_played = 6;
          }
        }
      }
    }


    if (!(Car.IsDead($.pred_boat))) {
      if (!(Char.IsDead($.rico_passenger))) {
        if ($.player1.isInCar($.pred_boat)) {
          Text.ClearThisPrint("CUB1_14");
          if ($.player1.locateAnyMeans3D($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz, 7.0, 7.0, 7.0, false /* FALSE */)) {
            ++$.boat_check_counter;
            --$.checks_left;
            Sound.AddOneOffSound(0.0, 0.0, 0.0, 1 /* SOUND_PART_MISSION_COMPLETE */);
            $.boatcheck_blip.remove();
            $.next_boatcheck_blip.remove();
            if ($.boat_check_counter == 1) {
              $.boatcheck_blipx = $.boat_check2x;
              $.boatcheck_blipy = $.boat_check2y;
              $.boatcheck_blipz = $.boat_check2z;
              $.next_boatcheck_blipx = $.boat_check3x;
              $.next_boatcheck_blipy = $.boat_check3y;
              $.next_boatcheck_blipz = $.boat_check3z;
            }
            if ($.boat_check_counter == 2) {
              $.boatcheck_blipx = $.boat_check3x;
              $.boatcheck_blipy = $.boat_check3y;
              $.boatcheck_blipz = $.boat_check3z;
              $.next_boatcheck_blipx = $.boat_check4x;
              $.next_boatcheck_blipy = $.boat_check4y;
              $.next_boatcheck_blipz = $.boat_check4z;
            }
            if ($.boat_check_counter == 3) {
              $.camerax = 369.99;
              $.cameray = 74.30;
              $.cameraz = 8.66;
              $.locate1x = 308.57;
              $.locate1y = 99.09;
              $.locate2x = 340.835;
              $.locate2y = 107.66;
              $.boatcheck_blipx = $.boat_check4x;
              $.boatcheck_blipy = $.boat_check4y;
              $.boatcheck_blipz = $.boat_check4z;
              $.next_boatcheck_blipx = $.boat_check5x;
              $.next_boatcheck_blipy = $.boat_check5y;
              $.next_boatcheck_blipz = $.boat_check5z;
            }
            if ($.boat_check_counter == 4) {
              $.fake_caddy.markAsNoLongerNeeded();
              $.fake_golfer1.markAsNoLongerNeeded();
              $.fake_golfer2.markAsNoLongerNeeded();
              $.fake_golfer3.markAsNoLongerNeeded();
              $.fake_golfer4.markAsNoLongerNeeded();
              $.boatcheck_blipx = $.boat_check5x;
              $.boatcheck_blipy = $.boat_check5y;
              $.boatcheck_blipz = $.boat_check5z;
              $.next_boatcheck_blipx = $.boat_check6x;
              $.next_boatcheck_blipy = $.boat_check6y;
              $.next_boatcheck_blipz = $.boat_check6z;
            }
            if ($.boat_check_counter == 5) {
              $.boatcheck_blipx = $.boat_check6x;
              $.boatcheck_blipy = $.boat_check6y;
              $.boatcheck_blipz = $.boat_check6z;
              $.next_boatcheck_blipx = $.boat_check7x;
              $.next_boatcheck_blipy = $.boat_check7y;
              $.next_boatcheck_blipz = $.boat_check7z;
            }
            if ($.boat_check_counter == 6) {
              $.boatcheck_blipx = $.boat_check7x;
              $.boatcheck_blipy = $.boat_check7y;
              $.boatcheck_blipz = $.boat_check7z;
              $.next_boatcheck_blipx = $.boat_check8x;
              $.next_boatcheck_blipy = $.boat_check8y;
              $.next_boatcheck_blipz = $.boat_check8z;
            }
            if ($.boat_check_counter == 7) {
              $.camerax = 396.14;
              $.cameray = -90.01;
              $.cameraz = 11.48;
              $.locate1x = 421.73;
              $.locate1y = -83.21;
              $.locate2x = 403.83;
              $.locate2y = -91.97;
              $.boatcheck_blipx = $.boat_check8x;
              $.boatcheck_blipy = $.boat_check8y;
              $.boatcheck_blipz = $.boat_check8z;
              $.next_boatcheck_blipx = $.boat_check9x;
              $.next_boatcheck_blipy = $.boat_check9y;
              $.next_boatcheck_blipz = $.boat_check9z;
            }
            if ($.boat_check_counter == 8) {
              $.camerax = -194.5;
              $.cameray = -783.5;
              $.cameraz = 9.2;
              $.locate1x = -183.166;
              $.locate1y = -756.844;
              $.locate2x = -181.80;
              $.locate2y = -780.26;
              $.boatcheck_blipx = $.boat_check9x;
              $.boatcheck_blipy = $.boat_check9y;
              $.boatcheck_blipz = $.boat_check9z;
              $.next_boatcheck_blipx = $.boat_check10x;
              $.next_boatcheck_blipy = $.boat_check10y;
              $.next_boatcheck_blipz = $.boat_check10z;
            }
            if ($.boat_check_counter == 9) {
              $.boatcheck_blipx = $.boat_check10x;
              $.boatcheck_blipy = $.boat_check10y;
              $.boatcheck_blipz = $.boat_check10z;
              $.next_boatcheck_blipx = $.boat_check11x;
              $.next_boatcheck_blipy = $.boat_check11y;
              $.next_boatcheck_blipz = $.boat_check11z;
            }
            if ($.boat_check_counter == 10) {
              $.boatcheck_blipx = $.boat_check11x;
              $.boatcheck_blipy = $.boat_check11y;
              $.boatcheck_blipz = $.boat_check11z;
              $.next_boatcheck_blipx = $.boat_check12x;
              $.next_boatcheck_blipy = $.boat_check12y;
              $.next_boatcheck_blipz = $.boat_check12z;
            }
            if ($.boat_check_counter == 11) {
              $.boatcheck_blipx = $.boat_check12x;
              $.boatcheck_blipy = $.boat_check12y;
              $.boatcheck_blipz = $.boat_check12z;
              $.next_boatcheck_blipx = $.boat_check13x;
              $.next_boatcheck_blipy = $.boat_check13y;
              $.next_boatcheck_blipz = $.boat_check13z;
            }
            if ($.boat_check_counter == 12) {
              $.boatcheck_blipx = $.boat_check13x;
              $.boatcheck_blipy = $.boat_check13y;
              $.boatcheck_blipz = $.boat_check13z;
              $.next_boatcheck_blipx = $.boat_check14x;
              $.next_boatcheck_blipy = $.boat_check14y;
              $.next_boatcheck_blipz = $.boat_check14z;
            }
            if ($.boat_check_counter == 13) {
              $.boatcheck_blipx = $.boat_check14x;
              $.boatcheck_blipy = $.boat_check14y;
              $.boatcheck_blipz = $.boat_check14z;
              $.next_boatcheck_blipx = $.boat_check15x;
              $.next_boatcheck_blipy = $.boat_check15y;
              $.next_boatcheck_blipz = $.boat_check15z;
            }
            if ($.boat_check_counter == 14) {
              if ($.del_ramps == 0) {
                $.ramp1.delete();
                $.ramp2.delete();
                $.ramp3.delete();
                //DELETE_OBJECT ramp4
                //MARK_CAR_AS_NO_LONGER_NEEDED fake_reefer
                $.ramp1 = ScriptObject.Create(waterjump1, 295.2, -446.2, 5.5);
                $.ramp1.setHeading(326.50);
                $.ramp1.setCollision(true /* TRUE */);
                $.ramp2 = ScriptObject.Create(waterjumpx2, 421.200, -112.669, 5.5);
                $.ramp2.setHeading(356.88);
                $.ramp2.setCollision(true /* TRUE */);
                $.del_ramps = 1;
              }
              $.boatcheck_blipx = $.boat_check15x;
              $.boatcheck_blipy = $.boat_check15y;
              $.boatcheck_blipz = $.boat_check15z;
              $.next_boatcheck_blipx = $.boat_check16x;
              $.next_boatcheck_blipy = $.boat_check16y;
              $.next_boatcheck_blipz = $.boat_check16z;
            }
            if ($.boat_check_counter == 15) {
              $.boatcheck_blipx = $.boat_check16x;
              $.boatcheck_blipy = $.boat_check16y;
              $.boatcheck_blipz = $.boat_check16z;
              $.next_boatcheck_blipx = $.boat_check17x;
              $.next_boatcheck_blipy = $.boat_check17y;
              $.next_boatcheck_blipz = $.boat_check17z;
            }
            if ($.boat_check_counter == 16) {
              $.boatcheck_blipx = $.boat_check17x;
              $.boatcheck_blipy = $.boat_check17y;
              $.boatcheck_blipz = $.boat_check17z;
              $.next_boatcheck_blipx = $.boat_check18x;
              $.next_boatcheck_blipy = $.boat_check18y;
              $.next_boatcheck_blipz = $.boat_check18z;
            }
            if ($.boat_check_counter == 17) {
              $.boatcheck_blipx = $.boat_check18x;
              $.boatcheck_blipy = $.boat_check18y;
              $.boatcheck_blipz = $.boat_check18z;
              $.next_boatcheck_blipx = $.boat_check19x;
              $.next_boatcheck_blipy = $.boat_check19y;
              $.next_boatcheck_blipz = $.boat_check19z;
            }
            if ($.boat_check_counter == 18) {
              $.boatcheck_blipx = $.boat_check19x;
              $.boatcheck_blipy = $.boat_check19y;
              $.boatcheck_blipz = $.boat_check19z;
              $.next_boatcheck_blipx = $.boat_check20x;
              $.next_boatcheck_blipy = $.boat_check20y;
              $.next_boatcheck_blipz = $.boat_check20z;
            }
            if ($.boat_check_counter == 19) {
              $.boatcheck_blipx = $.boat_check20x;
              $.boatcheck_blipy = $.boat_check20y;
              $.boatcheck_blipz = $.boat_check20z;
              $.next_boatcheck_blipx = $.boat_check21x;
              $.next_boatcheck_blipy = $.boat_check21y;
              $.next_boatcheck_blipz = $.boat_check21z;
            }
            if ($.boat_check_counter == 20) {
              $.boatcheck_blipx = $.boat_check21x;
              $.boatcheck_blipy = $.boat_check21y;
              $.boatcheck_blipz = $.boat_check21z;
              $.next_boatcheck_blipx = $.boat_check22x;
              $.next_boatcheck_blipy = $.boat_check22y;
              $.next_boatcheck_blipz = $.boat_check22z;
            }
            if ($.boat_check_counter == 21) {
              $.boatcheck_blipx = $.boat_check22x;
              $.boatcheck_blipy = $.boat_check22y;
              $.boatcheck_blipz = $.boat_check22z;
              $.next_boatcheck_blipx = $.boat_check23x;
              $.next_boatcheck_blipy = $.boat_check23y;
              $.next_boatcheck_blipz = $.boat_check23z;
            }
            if ($.boat_check_counter == 22) {
              $.boatcheck_blipx = $.boat_check23x;
              $.boatcheck_blipy = $.boat_check23y;
              $.boatcheck_blipz = $.boat_check23z;
              $.next_boatcheck_blipx = $.boat_check24x;
              $.next_boatcheck_blipy = $.boat_check24y;
              $.next_boatcheck_blipz = $.boat_check24z;
            }
            if ($.boat_check_counter == 23) {
              $.boatcheck_blipx = $.boat_check24x;
              $.boatcheck_blipy = $.boat_check24y;
              $.boatcheck_blipz = $.boat_check24z;
              $.next_boatcheck_blipx = $.boat_check25x;
              $.next_boatcheck_blipy = $.boat_check25y;
              $.next_boatcheck_blipz = $.boat_check25z;
            }
            if ($.boat_check_counter == 24) {
              $.boatcheck_blipx = $.boat_check25x;
              $.boatcheck_blipy = $.boat_check25y;
              $.boatcheck_blipz = $.boat_check25z;
              $.next_boatcheck_blipx = $.boat_check26x;
              $.next_boatcheck_blipy = $.boat_check26y;
              $.next_boatcheck_blipz = $.boat_check26z;
            }
            if ($.boat_check_counter == 25) {
              $.boatcheck_blipx = $.boat_check26x;
              $.boatcheck_blipy = $.boat_check26y;
              $.boatcheck_blipz = $.boat_check26z;
              $.next_boatcheck_blipx = $.boat_check27x;
              $.next_boatcheck_blipy = $.boat_check27y;
              $.next_boatcheck_blipz = $.boat_check27z;
            }
            if ($.boat_check_counter == 26) {
              $.boatcheck_blipx = $.boat_check27x;
              $.boatcheck_blipy = $.boat_check27y;
              $.boatcheck_blipz = $.boat_check27z;
            }
            if ($.boat_check_counter == 27) {
              $.player1.setControl(false /* OFF */);
              if (!(Car.IsDead($.pred_boat))) {
                $.pred_boat.setTempAction(5 /* TEMPACT_HANDBRAKESTRAIGHT */, 10000);
                $.pred_boat.setCruiseSpeed(0.0);
                $.pred_boat.anchor(true /* TRUE */);
              }

              //FADE OUT
              Camera.SetFadingColor(1, 1, 1);
              Camera.DoFade(1500, 0 /* FADE_OUT */);
              while (Camera.GetFadingStatus()) {
                await asyncWait(0);
              }
              Hud.ClearTimer($.$id.boatrace_end);
              await asyncWait(1000);
              if (!(Car.IsDead($.pred_boat))) {
                $.player1.warpFromCarToCoord(-79.3, 82.22, 8.6);
                $.player1.setHeading(120.75);
                if (!(Char.IsDead($.rico_passenger))) {
                  $.rico_passenger.detachFromCar();
                  $.rico_passenger.delete();
                  $.pred_boat.setCoordinates(-84.5, 82.7, 5.1);
                  $.pred_boat.setHeading(180.0);
                  $.rico_passenger = Char.CreateInsideCar($.pred_boat, 7 /* PEDTYPE_GANG_CUBAN */, CBb);
                }
              }


              Camera.DoFade(1500, 1 /* FADE_IN */);

              // ******************************************START OF PLAYER GETTING OUT OF BOAT*******************************************
              Hud.SwitchWidescreen(true /* ON */);
              Camera.SetFixedPosition(-75.82, 83.66, 11.59, 0.0, 0.0, 0.0);
              Camera.PointAtPoint(-80.32, 82.60, 9.96, 2 /* JUMP_CUT */);
              await asyncWait(1000);
              Audio.LoadMissionAudio(1, "CUB1_2" as any);
              while (!(Audio.HasMissionAudioLoaded(1))) {
                await asyncWait(0);
              }
              Text.PrintNow("CUB1_04", 4000, 1); //Men, you got big cajones, amigo.
              Audio.PlayMissionAudio(1);
              while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
                await asyncWait(0);
              }
              Audio.ClearMissionAudio(1);
              Text.ClearThisPrint("CUB1_04");
              if (!(Car.IsDead($.pred_boat))) {
                $.pred_boat.setForwardSpeed(40.0);
              }
              Streaming.MarkModelAsNoLongerNeeded(speeder);
              Streaming.MarkModelAsNoLongerNeeded(CBb);
              Camera.RestoreJumpcut();
              $.player1.setControl(true /* ON */);
              Hud.SwitchWidescreen(false /* OFF */);
              // ******************************************END OF PLAYER GETTING OUT OF BOAT*********************************************
              // SCM GOTO → mission_passed_cuban1 (not lowered; manual jump required)
              throw new Error("unresolved GOTO mission_passed_cuban1"); // fallback: would break linear control flow
            }
            $.boatcheck_blip = Blip.AddForCoord($.boatcheck_blipx, $.boatcheck_blipy, $.boatcheck_blipz);
            $.boatcheck_blip.changeColor(4 /* YELLOW */);
            $.next_boatcheck_blip = Blip.AddForCoord($.next_boatcheck_blipx, $.next_boatcheck_blipy, $.next_boatcheck_blipz);
            $.next_boatcheck_blip.changeColor(4 /* YELLOW */);
            $.next_boatcheck_blip.changeScale(2);
            $.next_boatcheck_blip.dim(true /* TRUE */);


          }
        }
        else {
          Text.PrintNow("CUB1_14", 5000, 1); //Get back in the boat!
        }
      }
      else {
        Text.PrintNow("CUB1_11", 5000, 1); //You Killed Rico!
        $.rico_passenger.delete();
        // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
      }
    }
    else {
      Text.PrintNow("CUB1_11", 5000, 1); //You Killed Rico!
      $.rico_passenger.delete();
      // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
      throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
    }


    if ($.boatrace_end == 0) {
      if (!(Car.IsDead($.pred_boat))) {
        Audio.LoadMissionAudio(1, "CUB1_6" as any);
        while (!(Audio.HasMissionAudioLoaded(1))) {
          await asyncWait(0);
        }
        Text.PrintNow("CUB1_08", 4000, 1); //You a big waste of space. Walk like a man, talk like a man, but you drive like and idiot.
        Audio.PlayMissionAudio(1);
        while (!(Audio.HasMissionAudioFinished(1)) && !(Char.IsDead($.rico_passenger))) {
          await asyncWait(0);
        }
        Audio.ClearMissionAudio(1);
        Text.ClearThisPrint("CUB1_08");
        if (!(Char.IsDead($.rico_passenger))) {
          //DETACH_CHAR_FROM_CAR rico_passenger
          $.rico_passenger.removeElegantly();
        }
        Text.PrintNow("CUB1_15", 5000, 1); //You are too slow, man.
        // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
      }
    }



    if (!(Car.IsDead($.pred_boat))) {
      if (!($.player1.locateAnyMeansCar2D($.pred_boat, 15.0, 15.0, false /* FALSE */))) {
        if (!(Char.IsDead($.rico_passenger))) {
          //DETACH_CHAR_FROM_CAR rico_passenger
          $.rico_passenger.removeElegantly();
        }
        Text.PrintNow("T4X4_F", 5000, 1); //You bailed! Too tough for you?!
        // SCM GOTO → mission_failed_cuban1 (not lowered; manual jump required)
        throw new Error("unresolved GOTO mission_failed_cuban1"); // fallback: would break linear control flow
      }
    }



  }
}





// Mission failed

async function mission_failed_cuban1() {



  Text.PrintBig("M_FAIL", 5000, 1); //"Mission Failed!"
}




// mission passed

async function mission_passed_cuban1() {


  Text.ClearPrints();
  Text.PrintWithNumberBig("M_PASS", 1000, 5000, 1); //"Mission Passed!"
  Audio.PlayMissionPassedTune(1);
  $.player1.clearWantedLevel();
  $.player1.addScore(1000);
  Stat.PlayerMadeProgress(1);
  Stat.RegisterMissionPassed("CUB_1");
  // START_NEW_SCRIPT cuban_mission2_loop 
  $.flag_cuban_mission1_passed = 1;
}




// mission cleanup

async function mission_cleanup_cuban1() {


  ONMISSION = 0;
  Path.SwitchRoadsOn(130.4, 368.4, 50.7, -228.4, 724.5, 0.0);
  $.player1.setControl(true /* on */);
  //IF flag_cuban_mission1_passed = 0

  $.ramp1.delete();
  $.ramp2.delete();
  $.ramp3.delete();
  //DELETE_OBJECT ramp4
  //ENDIF

  Streaming.MarkModelAsNoLongerNeeded(speeder);
  Streaming.MarkModelAsNoLongerNeeded(CBb);
  Streaming.MarkModelAsNoLongerNeeded(caddy);
  Streaming.MarkModelAsNoLongerNeeded(WMYGO);
  Streaming.MarkModelAsNoLongerNeeded(WFOGO);
  Streaming.MarkModelAsNoLongerNeeded(reefer);
  $.boatcheck_blip.remove();
  $.next_boatcheck_blip.remove();
  Hud.ClearTimer($.$id.boatrace_end);
  Hud.ClearCounter($.$id.checks_left);
  $.timer_mobile_start = Clock.GetGameTimer();
  Weather.Release();
  Mission.Finish();
}

export async function cuban1() {
  // MissionBoundary
  // *****************************************************************************************
  // *****************************************************************************************
  // *****************************************Cubans Mission 1********************************
  // *****************************************Boat Stunt Checkpoint mission*******************
  // *****************************************************************************************
  // *****************************************************************************************

  // SCRIPT_NAME cuban1
  // Mission start stuff

  await mission_start_cuban1();  // SCM GOSUB mission_start_cuban1
  if (HAS_DEATHARREST_BEEN_EXECUTED()) {
    await mission_failed_cuban1();  // SCM GOSUB mission_failed_cuban1
  }
  await mission_cleanup_cuban1();  // SCM GOSUB mission_cleanup_cuban1
  // MissionBoundary

  // Variables for mission
  //people, cars and objects

  // VAR_INT pred_boat
  // VAR_INT rico_passenger
  // VAR_INT ramp1
  // VAR_INT ramp2
  // VAR_INT ramp3
  //VAR_INT ramp4

  // VAR_INT ramp5
  // VAR_INT ramp6
  // VAR_INT preddy_boat
  // VAR_INT fake_caddy
  // VAR_INT fake_golfer1
  // VAR_INT fake_golfer2
  // VAR_INT fake_golfer3
  // VAR_INT fake_golfer4
  //VAR_INT fake_reefer

  //blips

  // VAR_INT boatcheck_blip
  // VAR_INT next_boatcheck_blip

  //flags

  // VAR_INT boatrace_end
  // VAR_INT boat_check_counter
  // VAR_INT del_ramps
  // VAR_INT jumpy_flag
  // VAR_INT boat_col_check
  // VAR_INT time_check
  // VAR_FLOAT boat_speed
  // VAR_INT starting_timer
  // VAR_INT cub1_speech_played
  // VAR_INT checks_left
  // VAR_INT golfer_hit1
  // VAR_INT golfer_hit2
  // VAR_INT golfer_hit3
  // VAR_INT golfer_hit4

  //coordinates

  // VAR_FLOAT boat_check1x
  // VAR_FLOAT boat_check1y
  // VAR_FLOAT boat_check1z
  // VAR_FLOAT boat_check2x
  // VAR_FLOAT boat_check2y
  // VAR_FLOAT boat_check2z
  // VAR_FLOAT boat_check3x
  // VAR_FLOAT boat_check3y
  // VAR_FLOAT boat_check3z
  // VAR_FLOAT boat_check4x
  // VAR_FLOAT boat_check4y
  // VAR_FLOAT boat_check4z
  // VAR_FLOAT boat_check5x
  // VAR_FLOAT boat_check5y
  // VAR_FLOAT boat_check5z
  // VAR_FLOAT boat_check6x
  // VAR_FLOAT boat_check6y
  // VAR_FLOAT boat_check6z
  // VAR_FLOAT boat_check7x
  // VAR_FLOAT boat_check7y
  // VAR_FLOAT boat_check7z
  // VAR_FLOAT boat_check8x
  // VAR_FLOAT boat_check8y
  // VAR_FLOAT boat_check8z
  // VAR_FLOAT boat_check9x
  // VAR_FLOAT boat_check9y
  // VAR_FLOAT boat_check9z
  // VAR_FLOAT boat_check10x
  // VAR_FLOAT boat_check10y
  // VAR_FLOAT boat_check10z
  // VAR_FLOAT boat_check11x
  // VAR_FLOAT boat_check11y
  // VAR_FLOAT boat_check11z
  // VAR_FLOAT boat_check12x
  // VAR_FLOAT boat_check12y
  // VAR_FLOAT boat_check12z
  // VAR_FLOAT boat_check13x
  // VAR_FLOAT boat_check13y
  // VAR_FLOAT boat_check13z
  // VAR_FLOAT boat_check14x
  // VAR_FLOAT boat_check14y
  // VAR_FLOAT boat_check14z
  // VAR_FLOAT boat_check15x
  // VAR_FLOAT boat_check15y
  // VAR_FLOAT boat_check15z
  // VAR_FLOAT boat_check16x
  // VAR_FLOAT boat_check16y
  // VAR_FLOAT boat_check16z
  // VAR_FLOAT boat_check17x
  // VAR_FLOAT boat_check17y
  // VAR_FLOAT boat_check17z
  // VAR_FLOAT boat_check18x
  // VAR_FLOAT boat_check18y
  // VAR_FLOAT boat_check18z
  // VAR_FLOAT boat_check19x
  // VAR_FLOAT boat_check19y
  // VAR_FLOAT boat_check19z
  // VAR_FLOAT boat_check20x
  // VAR_FLOAT boat_check20y
  // VAR_FLOAT boat_check20z
  // VAR_FLOAT boat_check21x
  // VAR_FLOAT boat_check21y
  // VAR_FLOAT boat_check21z
  // VAR_FLOAT boat_check22x
  // VAR_FLOAT boat_check22y
  // VAR_FLOAT boat_check22z
  // VAR_FLOAT boat_check23x
  // VAR_FLOAT boat_check23y
  // VAR_FLOAT boat_check23z
  // VAR_FLOAT boat_check24x
  // VAR_FLOAT boat_check24y
  // VAR_FLOAT boat_check24z
  // VAR_FLOAT boat_check25x
  // VAR_FLOAT boat_check25y
  // VAR_FLOAT boat_check25z
  // VAR_FLOAT boat_check26x
  // VAR_FLOAT boat_check26y
  // VAR_FLOAT boat_check26z
  // VAR_FLOAT boat_check27x
  // VAR_FLOAT boat_check27y
  // VAR_FLOAT boat_check27z
  // VAR_FLOAT boatcheck_blipx
  // VAR_FLOAT boatcheck_blipy
  // VAR_FLOAT boatcheck_blipz
  // VAR_FLOAT next_boatcheck_blipx
  // VAR_FLOAT next_boatcheck_blipy
  // VAR_FLOAT next_boatcheck_blipz
  // VAR_FLOAT camerax
  // VAR_FLOAT cameray
  // VAR_FLOAT cameraz
  // VAR_FLOAT locate1x
  // VAR_FLOAT locate1y
  // VAR_FLOAT locate2x
  // VAR_FLOAT locate2y

  // ****************************************Mission Start************************************

}
